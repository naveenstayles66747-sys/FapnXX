...
rl.on("line", (line) => {
  ...
  if (fullCategories === ALL_CATEGORIES.length || rowCount > 150000) {
    if (!isFinished) {
      isFinished = true;
...
rl.on("close", () => {
  ...
  fs.writeFileSync("src/data/pornhubCurated.json", JSON.stringify(uniqueVideos, null, 2), "utf8");
  fs.writeFileSync("public/data/videos_page1.json", JSON.stringify(uniqueVideos, null, 2), "utf8");
  console.log("Successfully written 1000+ curated videos to dataset files!");
});
