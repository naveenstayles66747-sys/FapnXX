...
async function buildMassiveDataset() {
  const csvPath = path.join(process.cwd(), "affiliate-webmaster", "pornhub.com-db", "pornhub.com-db.csv");
  console.log("Starting massive catalog builder from CSV...");

  const categoryBuckets = {};
  ALL_CATEGORIES.forEach(c => categoryBuckets[c] = []);
  const seenIds = new Set();
  const TARGET_PER_CATEGORY = 60; // 60 videos per category = ~1,500 videos total!
  ...
  for await (const line of rl) {
    rowCount++;
    if (rowCount === 1 && line.startsWith("embed_url|")) continue;
    ...
    if (!embedUrl || !title || viewsCount < 15000) continue;
    ...
    // Check if this video helps any hungry category bucket
    ...
    const performerName = actors.length > 0 ? actors[0] : (getCategoryLabel(hungryCat) + " Star");
    ...
    if (isAllFull) {
      console.log("All " + ALL_CATEGORIES.length + " categories reached " + TARGET_PER_CATEGORY + " videos!");
      break;
    }
  }

  // Consolidate unique videos
  const uniqueVideos = [];
  const added = new Set();
  ...
  fs.writeFileSync(dest1, JSON.stringify(uniqueVideos, null, 2), "utf8");
  fs.writeFileSync(dest2, JSON.stringify(uniqueVideos, null, 2), "utf8");
  console.log("Massive catalog successfully saved!");
}
