...
async function run() {
  ...
  const TARGET_PER_CATEGORY = 50; // 50 videos * 26 categories = 1,300 videos
  ...
  rl.on("line", (line) => {
    ...
    if (isAllFull) {
      console.log("All categories reached " + TARGET_PER_CATEGORY + " videos! Closing stream...");
      isClosed = true;
      rl.close();
      fileStream.destroy();
    }
  ...
  rl.on("close", () => {
    ...
    console.log("Massive catalog successfully saved to src/data/pornhubCurated.json and public/data/videos_page1.json!");
  });
}
